<template>
  ...
  <QuillEditor ref="myQuillEditor" theme="snow" v-model:content="content" :options="editorOption" contentType="html" />
...
import quillTool from '@/components/quillTool'
...
export default {
        ...
        modules: {
          ImageExtend: {
            ...
            action:
              ...
              '/api/merchant/upload/upload-image', // 服务器地址，如果为空则采用base64插入图片
            headers: (xhr) => {
              // 设置请求头参数（选填）
  ...
  methods: {
    save() {
      const editor = this.$refs.myQuillEditor
      console.log(editor)
